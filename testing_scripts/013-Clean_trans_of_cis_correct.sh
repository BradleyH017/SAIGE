...
# bsub -o logs/saige_tidy_trans-%J-%I-output.log -e logs/saige_tidy_trans-%J-%I-error.log -q normal -G team152 -n 1 -M 40000 -a "memlimit=True" -R "select[mem>40000] rusage[mem=40000] span[hosts=1]" -J "saige_tidy_trans[1-22]" < testing_scripts/013-Clean_trans_of_cis_correct.sh 
...
while read gene; do
        ...
        tail -n +2 ${catdir}/trans/${gene}__npc${n_expr_pcs}_trans_by_cis.txt >> ${catdir}/trans/chr${gene_chr}_nPC_${n_expr_pcs}_trans_by_cis.txt
done < ${catdir}/chr${gene_chr}_genes.txt

# Perform correction of the trans-by-cis-results
echo "Correcting the trans-results"
Rscript ${repo_dir}/testing_scripts/bin/correct_trans_by_cis.R -d "${catdir}/trans" -c "$gene_chr" -n "$n_expr_pcs" -w "$cis_window" -a "$annotation__file"
